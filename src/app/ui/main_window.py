...
class Window(QMainWindow):
    ...
    def apply_theme(self, is_startup=False):
        ...
        is_welcome_screen = self.stacked_widget.currentWidget() == self.pages.get("welcome")
        
        if dark_mode_enabled:
            self.central_widget.setStyleSheet(DARK_MODE_STYLES["background"])
            self.setStyleSheet(DARK_MODE_STYLES["app_stylesheet_dark"])
        else:
            background_style = APP_CONFIG["background_style"].replace("border-radius: 20px;", "border-radius: 0px;") \
                if is_welcome_screen else APP_CONFIG["background_normal"]
        ...
        for widget in self.line_edits:
            if isinstance(widget, QTextEdit):
                style = DARK_MODE_STYLES["text_edit"] if dark_mode_enabled else STYLES["text_edit_light"]
                widget.setStyleSheet(style)
            elif isinstance(widget, QLineEdit):
                style = DARK_MODE_STYLES["line_edit"] if dark_mode_enabled else STYLES["line_edit"]
                widget.setStyleSheet(style)
    ...
    def show_settings_ui(self):
        self._show_page("settings", setup_settings_page, self.apply_theme, show_home_button=True)
        self.history.clear()
        self._update_nav_buttons(show_back=True)
        self._update_nav_buttons(show_home=True)
