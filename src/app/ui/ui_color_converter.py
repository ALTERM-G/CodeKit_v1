...
from .custom_widgets import HoverSlider
...
def setup_color_converter_page(window_instance):
    ...
    def choose_color():
        ...
        picker_icon_path = os.path.join(window_instance.assets_dir, "icons", "palette.svg").replace("\\", "/")
        dialog.setCursor(window_instance.Cursor)
        for widget in dialog.findChildren(QWidget):
            if isinstance(widget, QPushButton) and "screen" in widget.text().lower():
        ...
        if dialog.exec():
            ...
            border_color = '#CCCCCC' if dark_mode else 'black'
            icon_path = window_instance.palette_svg_path if dark_mode else window_instance.palette_black_svg_path
            hover_bg_color = '#555555' if dark_mode else '#f0f0f0'

            window_instance.color_button.setStyleSheet(f"""
                QPushButton {{ background-color: {hex_color}; border: 2px solid {border_color}; border-radius: 6px; }}
                QPushButton:hover {{ background-color: {hover_bg_color}; icon: url({icon_path}); }}
            """)
