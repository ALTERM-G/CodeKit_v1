...
def generate_polynomial():
    ...
    if random.random() < 0.3:
        ...
        if a == 0 and d == 0: a = random.randint(1, 5) 
        if a == d: d += random.choice([-1, 1]) or 1
        left = _format_quadratic(a, b, c)
        right = _format_linear(d, e).replace('+', ' + ').replace('-', ' - ')
        return f"$${left} = {right}$$"
    else:
...
def generate_fractional():
    ...
    if random.random() < 0.5:
        return rf"$$\frac{{{numerator_term}}}{{{denominator}}} = {constant}$$"
    else:
        numerator_const = random.randint(1, 5)
        return rf"$$\frac{{{numerator_term}+{numerator_const}}}{{{denominator}}} = {constant}$$"
